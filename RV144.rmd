...
were gonna break up the data into 2 equal groups. then were going to count up how many got aids in one group and how many got it in the other group.

Build data set to make it easy. 
```{r}
mydata = c(rep(0,times=16277),rep(1, times=125))
keepTrak=numeric()
for (i in 1:1000)  {
  group=sample(mydata,size = 16402,replace = FALSE)
  front=sum(goup[1:8201])
  back=sum(group[1:8210:16402])
  keepTrak[i]= abs(front-back)
}
```
